        ...
        return (
            news.map((newsItem, idx) => (
                <div className={globalStyles.newsListContainer} key={idx}>
                    ...
                    <h4 className={globalStyles.heading4} >{stringifyDate(newsItem.date)}</h4>
                    <p className={globalStyles.paragraph}>{getExcerpt(newsItem.content)} <Link className={globalStyles.showArticle} href={`/news?_id=${newsItem._id}`}>More</Link></p>
